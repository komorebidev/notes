...
$VaultName = "vault-mn6xr8gs" #fetch automatically
$Subscription = "Azure for Students" #fetch automatically
$ResourceGroup = "students-rg" #fetch automatically
$SubscriptionId = "89ca3363-489e-48b8-b449-f61c1069d3c0" #fetch automatically
...
foreach($item in $backupItemsVM) {
	Disable-AzRecoveryServicesBackupProtection -Item $item -VaultId $VaultToDelete.ID -RemoveRecoveryPoints -Force #stop backup and soft delete Azure VM backup items
}
Write-Host "Disabled and soft deleted Azure VM backup items"

foreach($item in $backupItemsSQL) {
	Disable-AzRecoveryServicesBackupProtection -Item $item -VaultId $VaultToDelete.ID -RemoveRecoveryPoints -Force #stop backup and soft delete SQL Server in Azure VM backup items
}
...
Write-Host "Disabled auto-protection and soft deleted SQL protectable items"
...
Write-Host "soft deleted SQL Servers in Azure VM containers"

foreach($item in $backupItemsSAP) {
	Disable-AzRecoveryServicesBackupProtection -Item $item -VaultId $VaultToDelete.ID -RemoveRecoveryPoints -Force #stop backup and soft delete SAP HANA in Azure VM backup items
}
...
Write-Host "soft deleted SAP HANA in Azure VM containers"

foreach($item in $backupItemsAFS) {
	Disable-AzRecoveryServicesBackupProtection -Item $item -VaultId $VaultToDelete.ID -RemoveRecoveryPoints -Force #stop backup and soft delete Azure File Shares backup items
}
...
foreach($item in $backupServersMARS) {
	Unregister-AzRecoveryServicesBackupContainer -Container $item -Force -VaultId $VaultToDelete.ID #unregister MARS servers and soft delete corresponding backup items
}
Write-Host "soft deleted MARS Servers"

foreach($item in $backupServersMABS) {
	Unregister-AzRecoveryServicesBackupManagementServer -AzureRmBackupManagementServer $item -VaultId $VaultToDelete.ID #unregister MABS servers and soft delete corresponding backup items
}
Write-Host "soft deleted MAB Servers"

foreach($item in $backupServersDPM) {
	Unregister-AzRecoveryServicesBackupManagementServer -AzureRmBackupManagementServer $item -VaultId $VaultToDelete.ID #unregister DPM servers and soft delete corresponding backup items
}
Write-Host "soft deleted DPM Servers"
Write-Host "Ensure that you stop protection and soft delete backup items from the respective MARS, MAB and DPM consoles as well. Visit https://go.microsoft.com/fwlink/?linkid=2186234 to learn more." -ForegroundColor Yellow
...
$plainToken = [System.Net.NetworkCredential]::new('', $accessToken.Token).Password
$authHeader = @{
...
if ($VaultDeleted -eq $null) {
	Write-Host "Recovery Services Vault" $VaultName "successfully soft deleted."
}
#Finish
